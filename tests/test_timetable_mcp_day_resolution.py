...
def no_sessions(mocker):
    ...
    mocker.patch.object(tt.timetable_service, "get_room_schedule", return_value=[])
    mocker.patch.object(tt.timetable_service, "find_free_rooms", return_value=[])
    mocker.patch.object(tt.timetable_service, "list_rooms", return_value=["CEP-209"])


class TestCalendarLookupCost:
    def test_defaulting_to_today_resolves_the_day_once(self, calendar_calls, no_sessions):
        asyncio.run(tt.find_free_rooms())
    ...
    def test_an_explicit_day_needs_no_calendar_at_all(self, calendar_calls, no_sessions):
        ...
        asyncio.run(tt.find_free_rooms(day="Monday"))

        assert calendar_calls == []

    def test_an_explicit_date_resolves_once(self, calendar_calls, no_sessions):
        asyncio.run(tt.get_room_schedule("CEP-209", date="2026-08-07"))

        assert len(calendar_calls) == 1


class TestSubstitutionSurvivesTheEmptyPath:
    def test_room_with_no_sessions_still_reports_the_substitution(
        self, calendar_calls, no_sessions
    ):
        msg = asyncio.run(tt.get_room_schedule("CEP-209", date="2026-08-07"))
    ...
    def test_whole_week_lookup_reports_no_day(self, mocker, no_sessions):
        ...
        msg = asyncio.run(tt.get_room_schedule("CEP-209"))
